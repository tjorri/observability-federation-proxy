# Observability Federation Proxy Helm Chart

A Helm chart for deploying the Observability Federation Proxy, which enables centralized Grafana to query Loki and Mimir across multiple Kubernetes clusters.
...
| `clusterSecrets.create` | Create secret for kubeconfigs | `false` |
| `clusterSecrets.existingSecret` | Use existing secret for kubeconfigs | `""` |
| `clusterSecrets.kubeconfigs` | Kubeconfig contents (if create=true) | `{}` |
